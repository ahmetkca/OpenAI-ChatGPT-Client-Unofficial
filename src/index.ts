...
class HttpClient {
  ...
  public async request(callback: (req: ClientRequest, res: IncomingMessage) => void, writeData?: string): Promise<void> {
    

    const req = https.request(this._reqOptions, (res) => {
      callback(req, res);
    });
...
class ChatGPTAuth {
  ...
  async login() {
    ...
    await this._httpClient.request(async (req, res) => {
      ...
      res.on('end', async () => {
        ...
        if (res.statusCode === 200 && res.statusMessage === 'OK') {
          console.log('Auth first flow step success');
          await this.getCsrfToken(cookies);
        } else {
          console.error('Auth first flow step failed');
          throw new Error('Auth first flow step failed');
        }
      });

    });

    console.log("Cmonnnnnnn");  
  }
